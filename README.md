...
If you see errors like `module 'app.main' has no attribute ...` or `ModuleNotFoundError: authlib`, it usually means your active interpreter/venv is different from the one where dependencies were installed, or your local checkout is stale.
...
python3 -m pytest -q
python3 scripts/diagnose_test_env.py
```

The diagnostic script verifies package availability and checks key exported symbols in `app.main` so you can quickly detect environment drift.
...
## Contribution workflow (to reduce merge conflicts)

Before committing and opening a PR, always sync with the latest main branch:
...
Then run tests and create your commit/PR.
